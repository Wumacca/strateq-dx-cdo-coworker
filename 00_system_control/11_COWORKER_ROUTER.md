...
Before any Coworker-generated refresh or population of a reusable artefact (template, schema, interface or workbook), the router directs the Coworker through the revision-handshake in `00_system_control/16_METHOD_ARTEFACT_REGISTRY.md`: bind one client, read the client `METHOD_BASELINE.md`, read the central registry, resolve the latest **approved** artefact revision at the approved method commit, confirm the interface/specification version matches and the artefact is approved and not superseded, record the resolved artefact ID/revision/commit, and fail closed on a stale, missing, ambiguous or incompatible baseline. "Latest" is the current approved revision, never an unpinned branch or an uploaded file. The populated output is written only to the bound client repository; no client data or populated artefact is written back to the method repository, which is read-only during client work.
