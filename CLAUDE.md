...
- `00_system_control/16_METHOD_ARTEFACT_REGISTRY.md` — authority for the central reusable-artefact registry and the deterministic revision-handshake: the Coworker resolves the latest **approved** artefact revision against the bound client's `METHOD_BASELINE.md` before any generated refresh or populated artefact, records the resolved artefact ID/revision/commit, fails closed on a stale/missing/ambiguous/incompatible baseline, and writes the populated output only to the client repository. No client data is ever written back into Strateq DX; the method repository is read-only during client work.
...
| Programme / portfolio workbook interface (uploaded snapshot, reconciliation, or Coworker-generated refresh) | `02_coworker_artifact_interface/10_PROGRAMME_PORTFOLIO_WORKBOOK_INTERFACE.md`; `00_system_control/16_METHOD_ARTEFACT_REGISTRY.md`; `00_system_control/13_INITIATIVE_CONTROL_RECORD_SCHEMA.md`; `00_system_control/14_CLIENT_WORKSPACE_AND_REPORTING_PROTOCOL.md`; `00_system_control/15_CLIENT_CONTEXT_ISOLATION_STANDARD.md`; `05_source_of_truth/01_DIGITAL_ARTEFACT_GOVERNANCE_MODEL.md`. The workbook is a governed interface and working snapshot; the Confirmation-First Status Gate applies and it never overrides a controlled record. |
| Reusable artefact resolution / revision handshake (any Coworker-generated refresh or populated reusable artefact) | `00_system_control/16_METHOD_ARTEFACT_REGISTRY.md`; `00_system_control/15_CLIENT_CONTEXT_ISOLATION_STANDARD.md`; `00_system_control/14_CLIENT_WORKSPACE_AND_REPORTING_PROTOCOL.md`; `00_system_control/13_INITIATIVE_CONTROL_RECORD_SCHEMA.md`; `05_source_of_truth/01_DIGITAL_ARTEFACT_GOVERNANCE_MODEL.md`. Resolve the latest **approved** artefact revision against the client `METHOD_BASELINE.md`; fail closed on a stale, missing, ambiguous or incompatible baseline; write the populated output only to the bound client repository. |
...
| Any client session / initiative continuity / current status | `00_system_control/15_CLIENT_CONTEXT_ISOLATION_STANDARD.md`; `00_system_control/14_CLIENT_WORKSPACE_AND_REPORTING_PROTOCOL.md`; `00_system_control/16_METHOD_ARTEFACT_REGISTRY.md`; `00_system_control/13_INITIATIVE_CONTROL_RECORD_SCHEMA.md`; `02_coworker_artifact_interface/04_INITIATIVE_EVIDENCE_AND_DECISION_FILE_TEMPLATE.md` |
